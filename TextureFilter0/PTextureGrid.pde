 ...
 *
 * PTextureGrid defines a 2D grid to map and distort textures.
 * v0.6
 * Changes:
...
// Textured quad grid that can be used to an store uniform grid and the 
// texture coordinates corresponding to each point of the grid.
public class PTextureGrid
{
    public PTextureGrid(GL gl)
    {
        this.gl = gl;
        initGrid();
    }
    
    public PTextureGrid(GL gl, XMLElement xml)
    {
        this.gl = gl;
        initGrid(xml);
    }

    public void render(int w, int h, int l)
    {
        PTexturedPoint p0, p1;
        p0 = new PTexturedPoint();
        p1 = new PTexturedPoint();
    ...
    private void getPoint(int layer, int i, int j, PTexturedPoint tp)
    {
        if (layer < numLayers)
        {
            tp.u = gridDX[layer];
    ...
    {
        PTexturedPoint p = new PTexturedPoint();
	for (int k = 1; k <= l; k++)
	{
            getPoint(k, i, j, p);
    ...
    protected void initGrid()
    {
        ...
        gridDX[0] = 1.0 / float(resX - 1); // i.e.: 1.0
        gridDY[0] = 1.0 / float(resY - 1); // i.e.: 1.0
    }
    ...
    */
    protected void initGrid(XMLElement xml)
    {
        ...
        if (n == 1)
        {
            ...
            gridDX[0] = 1.0 / float(resX - 1);
            gridDY[0] = 1.0 / float(resY - 1);          
        }
    }
